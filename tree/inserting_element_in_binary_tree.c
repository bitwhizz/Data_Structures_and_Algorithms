...
*  To inser an elemnet , we can use the level order traversal and insert the element 
*  whereever we find the nodes whose left or right child is NULL.
 */
...
int InsertInBinaryTree(struct BinaryTreeNode *root,int data)
{
    struct BinaryTreeNode *newnode = createnode(data);
    ...
    while(!isEmptyQueue(q)){
        ...
        if(current->treeNode->left){
            ...
            current->treeNode->left = newnode;
            printf("previous node: %x , new node on left subtree: %x\n",current->treeNode,current->treeNode->left);
            DeleteQueue(q);
            return current->treeNode->left->data;
        }
        ...
        if(current->treeNode->right){
            ...
            current->treeNode->right = newnode;
            printf("previous node: %x , new node on right subtree: %x\n",current->treeNode,current->treeNode->right);
            DeleteQueue(q);
            return current->treeNode->right->data;
        }
    }
    DeleteQueue(q);
    return -1;
...
*/
  ...
  root->right->right->right = NULL;
   ...
   printf("Inserting element in binary tree would be  :  %d \n", InsertInBinaryTree(root,9));
